...
router.post('/', auth_1.authenticate, auth_1.authorizeStudent, async (req, res, next) => {
    try {
        const { error, value } = validation_1.applicationSchema.validate(req.body);
        if (error) {
        ...
        // Check eligibility
        const studentProfile = await prisma_1.prisma.profile.findUnique({
...
router.get('/my-applications', auth_1.authenticate, auth_1.authorizeStudent, async (req, res, next) => {
    try {
...
router.get('/opportunity/:opportunityId', auth_1.authenticate, async (req, res, next) => {
    try {
    ...
    try {
        ...
        if (status === 'PLACED') {
            await prisma_1.prisma.profile.update({
                ...
                data: {
                    isPlaced: true,
                    placedAt: application.opportunity.company?.name || application.opportunity.college?.name || 'Unknown'
                }
            });
        }
        res.json({
...
router.get('/stats/overview', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const stats = await prisma_1.prisma.application.groupBy({
            by: ['status'],
            where,
            _count: {
                status: true
        ...
        const totalApplications = await prisma_1.prisma.application.count({ where });
        res.json({
