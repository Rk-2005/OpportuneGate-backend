...
import { authenticate, AuthRequest, authorizeAdminOrCompany } from '../middleware/auth';
import { opportunitySchema } from '../utils/validation';
import { sendOpportunityNotification } from '../utils/email';
import { prisma } from '../lib/prisma';

type OpportunityType = 'INTERNSHIP' | 'FULL_TIME' | 'CONTEST' | 'HACKATHON';
...
// Create opportunity
router.post('/', authenticate, authorizeAdminOrCompany, async (req: AuthRequest, res, next) => {
  try {
    ...
    try {
      if (processedGroupId) {
        ...
        for (const member of groupMembers) {
          await sendOpportunityNotification(member.user.email, member.user.profile?.firstName || 'Student', opportunity.title);
        }
...
// Update opportunity
router.put('/:id', authenticate, authorizeAdminOrCompany, async (req: AuthRequest, res, next) => {
  try {
    const { id } = req.params;
...
// Delete opportunity
router.delete('/:id', authenticate, authorizeAdminOrCompany, async (req: AuthRequest, res, next) => {
  try {
    const { id } = req.params;
...
async function sendNotificationsToEligibleStudents(opportunity: any) {
  try {
